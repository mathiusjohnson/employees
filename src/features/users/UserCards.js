...
const UserCards = () => {
  ...
  const renderedUsers = users.map(user => (
    <div class="p-10 mt-20 w-96">
      <Card class="flex flex-col"> 
        <Card.Body class="h-96 bg-square-tiles bg-cyan">
        <Card.Img class="-mt-24 object.contain h-48 w-48 mx-auto rounded-full h-16 w-16" variant="top" src={user.photo} />
        <div class="flex flex-col space-y-4 mt-8 text-center">
          <div class="mx-auto inline-block align-middle">
            <h5 class="uppercase text-turquoise font-bold">{user.first_name} {user.last_name}
              ...
              <p class="w-72 h-10 text-slateblue">{user.position}</p>
